...
description: >
  ...
  generates fully sequenced, dependency-aware tracks with cross-cutting
  constraints embedded in every spec.
...
4. **Generate** — Produce Conductor-compatible specs, plans, and metadata for every track
...
- `track-spec.md` — Per-track specification (with context header)
- `track-plan.md` — Per-track implementation plan
- `track-metadata.json` — Per-track state and configuration
- `context-header.md` / `context-header-minimal.md` — Compressed context for specs
...
- Every spec.md starts with a compressed context header (generated by inject_context.py)
...
3. After generating the track list before writing spec/plan files (Step 5 of decompose)
