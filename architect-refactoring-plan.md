...
```
architect-plugin/
├── .claude-plugin/plugin.json          ✅ keep as-is
├── commands/
│   ├── architect-decompose.md          🔄 MAJOR REWRITE
│   ├── architect-sync.md               🔄 MODERATE CHANGES
│   └── architect-status.md             ✅ minor tweaks
├── agents/
│   └── architect-expert.md             🔄 MODERATE CHANGES
├── skills/architect/
│   ├── SKILL.md                        🔄 MODERATE CHANGES
│   ├── references/
│   │   ├── architecture-patterns.md    ✅ keep as-is
│   │   ├── cross-cutting-catalog.md    ✅ keep as-is
│   │   └── classification-guide.md     ✅ keep as-is
│   └── templates/
│       ├── context-header.md           ✅ keep as-is
│       ├── context-header-minimal.md   ✅ keep as-is
│       ├── architecture.md             ✅ keep as-is
│       ├── cross-cutting.md            ✅ keep as-is
│       ├── interfaces.md              ✅ keep as-is
│       ├── dependency-graph.md         ✅ keep as-is
│       ├── execution-sequence.md       ✅ keep as-is
│       ├── track-spec.md              ❌ DELETE (Conductor generates specs)
│       ├── track-plan.md              ❌ DELETE (Conductor generates plans)
│       ├── track-metadata.json         🔄 SIMPLIFY
│       ├── patch-phase.md             ✅ keep as-is (patches still injected by Architect)
│       └── track-brief.md            ➕ NEW (the handoff artifact)
├── hooks/project-hooks/
│   ├── README.md                       🔄 MODERATE CHANGES
│   ├── constraint-update-check.md      ✅ keep as-is
│   ├── interface-verification.md       ✅ keep as-is
│   ├── discovery-check.md             ✅ keep as-is
│   ├── phase-validation.md            ✅ keep as-is
│   └── wave-sync.md                   ✅ keep as-is
├── scripts/
│   ├── validate_dag.py                ✅ keep as-is
│   ├── topological_sort.py            ✅ keep as-is
│   ├── inject_context.py             🔄 MODERATE (generates brief headers, not spec headers)
│   ├── merge_discoveries.py           ✅ keep as-is
│   ├── sync_check.py                  ✅ keep as-is
│   ├── validate_wave_completion.py    ✅ keep as-is
│   ├── check_conductor_compat.py      ✅ keep as-is
│   ├── progress.py                    ✅ keep as-is
│   └── regenerate_specs.py           ❌ DELETE (specs not generated by Architect)
├── README.md                          🔄 UPDATE descriptions
└── examples/sample-project/           🔄 UPDATE (briefs not specs)
```
...
RIGHT (identifying the decision):
  "Key Design Decision: Shell spawning — $SHELL vs hardcoded? Fallback?"
...
RIGHT (scoping the area):
  ...
  "Design Decision: Threading model — reader-only vs reader+writer?"
...
- NEW_TRACK discovery → generates a brief.md (not spec + plan)
- TRACK_EXTENSION discovery → generates a patch phase appended to plan.md
  (this still works — by the time a track needs a patch, Conductor has already
  generated plan.md, so Architect appends to it)
- CROSS_CUTTING_CHANGE → still appends to cross-cutting.md, still updates
...
- Add instruction: "Your output per track is a brief — scope, decisions,
...
- Update "What Gets Generated" section:
  ```
  conductor/tracks/{id}/brief.md      ← NEW (Architect's handoff)
  conductor/tracks/{id}/metadata.json ← Simplified
  conductor/tracks/{id}/spec.md       ← Generated by Conductor at pickup
  conductor/tracks/{id}/plan.md       ← Generated by Conductor from spec
  ```
...
3. During implementation, hooks read constraints from cross-cutting.md
   directly (not from the header — the header is for orientation)
```
...
its own interactive spec generation — asking better questions because
...
```
Create skills/architect/templates/track-brief.md — the new handoff
...
2. Generate metadata.json (simplified — no test_command, no test_timeout)
...
1. commands/architect-sync.md — NEW_TRACK generates brief.md not spec+plan
2. commands/architect-status.md — description updates
3. agents/architect-expert.md — system prompt: generates briefs not specs
4. skills/architect/SKILL.md — update template list, scripts list, "What Gets Generated"
5. scripts/inject_context.py — target is brief.md not spec.md
6. templates/track-metadata.json — remove test_command, test_timeout_seconds
7. hooks/project-hooks/README.md — add context header lifecycle note
8. README.md — update workflow description
...
"refactor: brief-based handoff model — Architect generates briefs, Conductor generates specs and plans"
```
...
If examples/ doesn't exist yet, skip this — we'll generate it later.
```
...
1. Search the entire repo for any remaining references to:
   ...
   - "generate spec.md" in the context of Architect generating it
     (should be zero — only references should be Conductor generating it)
...
interfaces, enforce token cap) is the same — just targeting a different file.
...
1. ✅ Architecture research phase (signal extraction, pattern matching)
2. ✅ Cross-cutting concern generation and versioning
3. ✅ Interface contract tracking
4. ✅ Dependency graph and wave sequencing
5. ✅ Discovery system (per-file, merge, dedup, conflict detection)
6. ✅ All 5 workflow hooks (unchanged)
7. ✅ Patch injection for retroactive compliance (still appends to plan.md
   AFTER Conductor has generated it)
8. ✅ Wave completion validation
9. ✅ Progress calculation
10. ✅ Reference knowledge base (architecture-patterns, cross-cutting-catalog,
    classification-guide)
11. ✅ Context header format (same ARCHITECT CONTEXT block)
12. ✅ Track state machine (NOT_STARTED → IN_PROGRESS → COMPLETE etc.)
13. ✅ Conductor compatibility checking
...
The refactor is surgical — most of the plugin stays exactly as built.
