...
5. **Read template:** `${CLAUDE_PLUGIN_ROOT}/skills/architect/templates/track-brief.md`
...
If the project has 25+ tracks and the orchestrator's context is genuinely at risk, fall back to dispatching brief-generator sub-agents (agent: `${CLAUDE_PLUGIN_ROOT}/agents/brief-generator.md`) in batches of 3-5. Pass the FULL per-track requirements (not a compressed bundle) via a JSON file written to disk:
```bash
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/validate_requirements.py \
