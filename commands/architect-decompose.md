...
1. **pattern-matcher** (agent: `${CLAUDE_PLUGIN_ROOT}/agents/pattern-matcher.md`):
...
2. **codebase-analyzer** (agent: `${CLAUDE_PLUGIN_ROOT}/agents/codebase-analyzer.md`):
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/prepare_brief_context.py --track <track_id> --tracks-dir conductor/tracks --architect-dir architect
```
...
Spawn **brief-generator** sub-agents (agent: `${CLAUDE_PLUGIN_ROOT}/agents/brief-generator.md`) in batches of 3-5 at a time:
...
6. Generate and write `conductor/tracks/<track_id>/metadata.json`:
   - Read template: `${CLAUDE_PLUGIN_ROOT}/skills/architect/templates/track-metadata.json`
