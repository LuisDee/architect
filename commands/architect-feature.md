...
The feature description is provided as the command argument: `$ARGUMENTS`

---

## Context Optimization Strategy

This command is lighter than `/architect-decompose`. It does NOT re-run architecture research — the architecture already exists. It reuses the existing `brief-generator` sub-agent for brief creation and runs the new `scope_analyzer.py` and `feature_context.py` scripts for analysis.
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/feature_context.py \
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/scope_analyzer.py \
    --feature "$ARGUMENTS" \
...
```bash
echo '<context_json>' | python ${CLAUDE_PLUGIN_ROOT}/scripts/scope_analyzer.py
```
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/validate_dag.py \
...
### 5b. Prepare Context and Generate Briefs

**Parallel execution (if Task tool is available — Claude Code):**

Spawn **brief-generator** sub-agents (agent: `${CLAUDE_PLUGIN_ROOT}/agents/brief-generator.md`) for each new track:

1. Run `prepare_brief_context.py` for each track with CLI overrides (metadata.json doesn't exist yet):
   ```bash
   python ${CLAUDE_PLUGIN_ROOT}/scripts/prepare_brief_context.py \
       --track <track_id> \
       --tracks-dir conductor/tracks \
       --architect-dir architect \
       --track-name "<name>" \
...
2. Spawn brief-generator sub-agent with the context bundle
3. Each sub-agent writes `brief.md` + `metadata.json` to disk
4. Collect one-line summaries

**Sequential fallback (if Task tool is unavailable — Gemini CLI):**

For each track:
1. Run `prepare_brief_context.py` to get context bundle
2. Run `inject_context.py` to generate context header
3. Read template: `${CLAUDE_PLUGIN_ROOT}/skills/architect/templates/track-brief.md`
4. Generate `brief.md` with:
   - **What This Track Delivers** — one paragraph
   - **Scope IN** — concrete boundaries
   - **Scope OUT** — what's excluded
   - **Key Design Decisions** — numbered questions for the developer (3-7)
   - **Architectural Notes** — integration points, cross-track impacts
   - **Complexity** — S/M/L/XL
   - **Estimated Phases** — advisory count
5. Write `brief.md` and `metadata.json` to `conductor/tracks/<track_id>/`
...
```bash
python ${CLAUDE_PLUGIN_ROOT}/scripts/generate_diagrams.py \
...
```
Feature decomposition complete: "$ARGUMENTS"
...
| Sub-agents | pattern-matcher + codebase-analyzer + brief-generator | brief-generator only |
