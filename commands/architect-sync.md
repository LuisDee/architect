...
2. Append a new phase using the patch-phase template: `${CLAUDE_PLUGIN_ROOT}/skills/architect/templates/patch-phase.md`
3. Update metadata.json: add entry to patches array
4. If track status is "completed", update status to "needs_patch"

**NEW_DEPENDENCY:**
1. Validate the edge won't create a cycle:
   ```bash
   python ${CLAUDE_PLUGIN_ROOT}/scripts/validate_dag.py --check-edge --from <dependent> --to <dependency>
   ```
...
2. Build retroactive compliance table:
   - "new" tracks: will auto-inherit via context header regeneration
   - "in_progress" tracks: will pick up via constraint-update-check hook
   - "completed" tracks: generate patch phase, set status to "needs_patch"
3. Regenerate context headers in brief.md for "new" tracks:
   ```bash
...
- **CC version drift:** Tracks running against stale CC versions. "in_progress" tracks should have picked up changes via Hook 01. "completed" tracks need patches.
